...
Vediamo ora invece
$$ ( \text{valid}(M) )^C = \Delta^* \setdiff \text{valid}(M) $$
...
\begin{teorema}
	$(\text{valid}(M))^C \in \text{CFL}$ ed esiste un algoritmo che, data $M$ costruisce una CFG o un PDA per $(\text{valid}(M))^2$.
\end{teorema}
...
\begin{corollarion}[Problema dell'universalità]
	Dato $L \in \text{CFL}$ non è possibile determinare se $L = \Sigma^*$.
\end{corollario}
